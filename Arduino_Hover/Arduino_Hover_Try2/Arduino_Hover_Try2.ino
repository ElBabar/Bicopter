...
void GetAllTilt(void) {
  ...
  compX = 0.99*(angleGyroX)+0.01*(-avgAccY/100);
  compY = 0.99*(angleGyroY)+0.01*(-angleAccX/100);
...
void averageAccY(void)
{
  ...
  long tempy;
  tempy=0;
  for(i=9; i>0; i--)      { histAccY[i] = histAccY[i-1]; }
  histAccY[0] = angleAccY;
  for(i=9; i>=0; i--)     { tempy = (tempy + histAccY[i]); }
  //Serial.print("                       ");
